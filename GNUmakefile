...
elf90: ## Compile Daphne and run tests for ELF90
	make checkone FC='wine elf90' FFLAGS='-npause -fullwarn' OBIN='tests.exe' OFLAG='-out tests.exe' ORUN='wine tests.exe && test ! -f error.log' FPPFLAGS='-D__ELF90__ -D__DP__ -D__GIT__=\"$(shell git rev-parse HEAD)\"' SRC='daphne.f90 tests.f90'
...
g95: ## Compile Daphne and run tests for g95 -std=F
	make tests FC='g95' FFLAGS='-std=F -S' OFLAG='' FPPFLAGS='-D__F__ -D__GIT__=\"$(shell git rev-parse HEAD)\"' SRC='daphne.f90 tests.f90'
...
ifx: ## Compile Daphne and run tests for ifx
	make checkone FC=ifx FFLAGS='-fpp -D__GIT__=\"$(shell git rev-parse HEAD)\" -warn errors -warn all -diag-error=remark,warn,error -O0 -g -traceback -fpe0 -fltconsistency -stand:f95 -debug full -diag-error-limit=1'

.PHONY: flang-7
flang-7: ## Compile Daphne and run tests for flang-7
	make checkone FC=flang-7 FFLAGS='-cpp -D__DP__ -D__GIT__=\"$(shell git rev-parse HEAD)\" -g -Wdeprecated'

.PHONY: sunf95
sunf95: ## Compile Daphne and run tests for sunf95
	make checkone FC=sunf95 FFLAGS='-fpp -D__GIT__=\"$(shell git rev-parse HEAD)\" -g -w4 -errwarn=%all -e -fnonstd -stackvar -ansi -C -fpover -xcheck=%all -U'
...
lfortran: ## Compile Daphne and run tests for lfortran
	make tests FC=lfortran FFLAGS='-c --cpp -D__DP__ -D__F__ -D__GIT__=\"$(shell git rev-parse HEAD)\"'
	lfortran daphne.o tests.o
	make checkone
...
openf95: ## Compile Daphne and run tests for openf95
	make tests FC=openf95 FFLAGS='-c -D__DP__ -D__GIT__=\"$(shell git rev-parse HEAD)\" -fullwarn -Wuninitialized'

.PHONY: fl32
fl32: ## Compile Daphne and run tests for fl32
	make checkone FC='wine fl32' FFLAGS='/4L132 /4Yb /4Yd /WX /4Yf' OBIN='tests.exe' OFLAG='/Fetests.exe' ORUN='wine tests.exe' FPPFLAGS='-D__DP__ -D__GIT__=\"$(shell git rev-parse HEAD)\"' SRC='daphne.f90 tests.f90'

.PHONY: absoft
absoft: ## Compile Daphne and run tests for absoft
	make checkone FC='wine f95' FFLAGS='-en -ea -Rb -Rc -Rs -Rp' OBIN='tests.exe' OFLAG='-o tests.exe' ORUN='wine tests.exe' FPPFLAGS='-D__DP__ -D__GIT__=\"$(shell git rev-parse HEAD)\"' SRC='daphne.f90 tests.f90'
...
ifl: ## Compile Daphne and run tests for ifl
	make checkone FC='wine ifl' FFLAGS='/CA /CB /CS /CU /4Yd /4Ys' OBIN='tests.exe' OFLAG='/Fetests.exe' ORUN='wine tests.exe' FPPFLAGS='-D__DP__ -D__GIT__=\"$(shell git rev-parse HEAD)\"' SRC='daphne.f90 tests.f90'

# Using the cvf preprocessor causes Wine to crash. /fpp:"__DP__"
.PHONY: cvf
cvf: ## Compile Daphne and run tests for cvf
	make checkone FC='wine f90' FFLAGS='/check:all /stand:f90 /warn:all' OBIN='tests.exe' OFLAG='/exe:tests.exe' ORUN='wine tests.exe' FPPFLAGS='-D__DP__ -D__GIT__=\"$(shell git rev-parse HEAD)\"' SRC='daphne.f90 tests.f90'
...
ftn95: $(SRC_FPP) ## Compile Daphne and run tests for ftn95
	...
	gfortran -E -D__DP__ -D__GIT__=\"$(shell git rev-parse HEAD)\" daphne.F90 | grep -v '^#' > daphne.f95
	gfortran -E -D__DP__ -D__GIT__=\"$(shell git rev-parse HEAD)\" tests.F90 | grep -v '^#' > tests.f95
